...
var Calendar = function () {
    ...
    this.drawHeader = function () {
        ...
        function drawButton(parameter) {
            var button = document.createElement("button");
            button.innerText = parameter[0];
        ...
        function drawInput(parametr) {
            var input = document.createElement("input");
            input.classList.add("input")
            headerContainer.appendChild(input)
            if (parametr[0] == 'output') {
                input.setAttribute('disabled', false);
                input.classList.add(parametr[0])
            } else {
        ...
        function init() {
            drawButton(previous);drawButton(next)
            drawInput(out);
            drawInput(inn);
    ...
    this.drawCalendar = function () {
        ...
        function drawWeekDays() {
            ...
            weakDays.classList.add("days-container");
        ...
        function drawDaysTable() {
            ...
            //event show selected day
            daysContainer.addEventListener("click",function(event){
                if(!event.toElement.classList.contains("day--empty")){
                this_.setDate(this_.year,this_.month,event.toElement.innerHTML);
                this_.update()}
            })
            for (var i = 1; i <= 42; i++) {
